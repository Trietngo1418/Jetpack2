...
trophy_spawn_threshold = 3000  # Initial spawn at 20,000 meters
...
TROPHY_EFFECT_DURATION = fps * 10  # 10 seconds (fps frames)
a_tier_pending = False  # Indicates if A Tier should spawn next after collecting B Tier
trophy_speed = 5
...
def spawn_trophy(tier='B'):
    ...
    # Update the last spawn distance
    last_trophy_spawn_distance = distance
    print(f"Spawned {tier} Tier trophy at distance {distance} meters.")

    def update_and_draw_trophy():
        global trophies, trophy_display_active, trophy_display_timer, a_tier_pending
        ...
        for trophy in trophies[:]:  # Loop through a copy of the list
            ...
            if player_rect.colliderect(trophy_rect):
                activate_trophy()
                trophies.remove(trophy)
                print(f"Collected {trophy['type']} Tier trophy at distance {distance} meters.")

                if trophy['type'] == 'B':
                    a_tier_pending = True  # Set A Tier to spawn next after 15,000 meters
                elif trophy['type'] == 'A':
                    pass
...
if trophy_display_active:
    ...
    # Determine which trophy image to display based on last collected trophy
    if a_tier_pending:
        # Last collected trophy was B Tier
        trophy_image = trophy_images['B']
    else:
        # Last collected trophy was A Tier
        trophy_image = trophy_images['A']

    if trophy_image:
        # Scale the trophy image to cover most of the screen
        scaled_image = pygame.transform.scale(trophy_image, (int(WIDTH * 0.8), int(HEIGHT * 0.8)))
...
while run:
    ...
    # Speed multiplier
    speed_multiplier = 1 if not hand_watch_active or  else 0.5
    ...
    if restart_cmd:
        ...
        hand_watch_timer = 0
        current_trophy = None
        trophy_spawn_threshold = 20000
        a_tier_pending = False
